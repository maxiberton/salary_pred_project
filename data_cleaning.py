...
def clean_and_sep_est_salary(df):
    ...
    df.loc[df.est_salary == 'Notgiven', 'est_salary'] = float('nan')

    df['min_salary'] = df.est_salary.str.split('-', expand=True)[0]
    df['min_salary'] = df['min_salary'].astype(float)
    df['max_salary'] = df.est_salary.str.split('-', expand=True)[1]
...
def get_state_for_each_location(df):
    df['state'] = df.location.apply(lambda x: x.split(',')[1].strip() if x != 'Remote' else 'Remote')
    return df
